		...
		struct page *old_page)
{
	...
	if (likely(pte_same(*fe->pte, orig_pte))) {
		...
		if(mm && (mm->badger_trap_en==1) && (!(fe->flags & FAULT_FLAG_INST)))
		{
...
int do_swap_page(struct fault_env *fe, pte_t orig_pte)
{
	...
	/* Make the page table entry as reserved for TLB miss tracking */
	if(vma->vm_mm && (vma->vm_mm->badger_trap_en==1) && (!(fe->flags & FAULT_FLAG_INST)))
	{
...
{//jie: may need to posion the pte entry bit
	struct vm_area_struct *vma = fe->vma;
	struct vm_fault vmf;
	int ret;
...
static int handle_pte_fault(struct fault_env *fe)
{
	...
	if (unlikely(pmd_none(*fe->pmd))) {
		...
		{
	                if(is_pte_reserved(*fe->pte))
	                        *fe->pte = pte_unreserve(*fe->pte);
		}
		 ...
		 * 2. We have taken a fake page fault on a normal page.
	 	 */
		if(mm && mm->badger_trap_en && (!(flags & FAULT_FLAG_INST)) && pte_present(*fe->pte))
		{
			page_table = pte_offset_map_lock(mm, fe->pmd, fe->address, &fe->ptl);
			entry = *page_table;
			if((flags & FAULT_FLAG_WRITE) && is_pte_reserved(entry) && !pte_write(entry))
			{
				pte_unmap_unlock(page_table, fe->ptl);
			...
			{
				return do_fake_page_fault(mm, vma, fe->address,
							 page_table, fe->pmd, flags, fe->ptl);
			}
	...
	pmd = pmd_offset(pud,address);
 	 ...
 	 * Here we check for transparent huge page that are marked as reserved
        ...
        if(mm && mm->badger_trap_en && (!(flags & FAULT_FLAG_INST)) && pmd && pmd_trans_huge(*pmd))
        {
			...
			return do_huge_pmd_wp_page(&fe, *pmd);
                }
